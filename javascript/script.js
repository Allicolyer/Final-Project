$(document).ready(function() {
  ...
  const words = {
    happy: ["happy ", "excited ", "elated ", "joyful ", "trilled ", "stoked "],
  ...
  function searchWords(word, array, response, city) {
    for (var k = 0; k < response.statuses.length; k++) {
      ...
      if (n != -1 && n != "undefined") {
        ...
        (obj.link = `https://twitter.com/ ${
          response.statuses[k].user.name
        }/status/${response.statuses[k].id_str}`),
        ...
        if (!response.statuses[k].coordinates) {
          obj.long = cities[city].longitude + 0.1 * (Math.random() - 0.5);
          obj.lat = cities[city].latitude + 0.1 * (Math.random() - 0.5);
        } else {
  ...
  var renderCity = function(city) {
    ...
    function renderMap() {
      ...
      for (i in markers) {
        ...
        var popUpTemplate =
          '<div class="content"><a target=_blank href="{{link}}">{{content}}</a></div>';
